...
/*          _this.authorized = false;
            ...
            */
            ...
            _this.fullName = message.params[0];
            _this.workerPass = message.params[1];

        } else {
            version = message.params[0];        
            _this.workerName = message.params[1].split('.')[0];
            _this.fullName = message.params[1];
            _this.workerPass = message.params[2];
        }

        majorver = version.split('.')[0];
        minorver = version.split('.')[1];
        if ( majorver < 3 ) {
            _this.authorized = false;
            sendJson({
                id     : message.id,
        ...
        options.authorizeFn(_this.remoteAddress, options.socket.localPort, _this.workerName, _this.workerPass, function(result) {
            ...
            if ( _this.authorized === true ) {
                _this.emit('setFullname', _this.subscriptionId, _this.fullName);
            }
    ...
    function setupSocket(){
        ...
        socket.on('data', function(d){
            offset = d.indexOf('mining.hello');
            if ( offset !== -1 && _this.usessl ) {
                dataBuffer = d.substr(offset - 21);
            } else {
                dataBuffer += d;
            }
...
var StratumServer = exports.Server = function StratumServer(options, authorizeFn){
    ...
    this.handleNewClient = function (socket, ssl){
    ...
    (function init(){
        ...
        if (options.banning && options.banning.enabled){
            setInterval(function(){
                for (ip in bannedIPs){
                    ...
                    if (Date.now() - banTime > options.banning.time * 1000 )
                        delete bannedIPs[ip];
                }
        ...
        if ((typeof(options.tlsOptions) !== 'undefined' && typeof(options.tlsOptions.enabled) !== 'undefined') && (options.tlsOptions.enabled === "true" || options.tlsOptions.enabled === true)) {
            TLSoptions = {
                ...
                handshakeTimeout: options.tlsOptions.handshakeTimeout,
                requireCert: false
            }
        }
        ...
        for (var port in options.ports) {
            if (options.ports[port].tls === false || options.ports[port].tls === "false") {
                net.createServer({allowHalfOpen: false}, function(socket) {
                    _this.handleNewClient(socket, false);
                ...
                this.server = tls.createServer(TLSoptions, function(socket) {
                    _this.handleNewClient(socket, true);
                }).listen(parseInt(port), function() {
                    serversStarted++;
                    if (serversStarted == Object.keys(options.ports).length)
                        _this.emit('started');
                });
                this.server.on('tlsClientError',function(error, socket){
                        _this.emit('tlsclientError', socket.remoteAddress + ': ' + error);
                        _this.addBannedIP(socket.remoteAddress);
                        socket.destroy();
                });
            }
        }
    ...
    this.addBannedWorkers = function(workers){
        for ( var id in stratumClients) {
            var client = stratumClients[id];
            if ( workers.includes(client.fullName) ) {
                _this.addBannedIP(client.remoteAddress);
                _this.removeStratumClientBySubId(client.subscriptionId);
                client.emit('checkBan');
            }
        }
    };
    ...
    this.setClientFullname = function(subId, name) {
        if ( typeof(stratumClients[subId]) !== 'undefined') {
            stratumClients[subId].fullName = name;
        }
    };
};
