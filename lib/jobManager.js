...
var ExtraNonceCounter = function () {
    var counter = crypto.randomBytes(4).readUIntLE(0, 4);
    this.next = function () {
        var extraNonce = new Buffer(8);
        extraNonce.writeUIntLE(crypto.randomBytes(4).readUIntLE(0, 4), 0, 4);
        extraNonce.writeUIntLE(counter++, 4, 4);
        if(counter >= 0xDFFFFFFF) counter = crypto.randomBytes(4).readUIntLE(0, 4);
...
var JobManager = module.exports = function JobManager(options) {
    ...
    this.processShare = function (jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, soln) {
        ...
        if (hashDigest(headerBuffer, new Buffer(soln.slice(6), 'hex')) !== true && nTime.length !== 8) {
            return shareError([20, 'invalid solution']);
        }
        //check if block candidate
        if (headerBigNum.le(job.target)) {
            console.log("There is a new block candidate");
