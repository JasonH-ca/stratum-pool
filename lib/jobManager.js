...
var JobManager = module.exports = function JobManager(options) {
    ...
    this.processTemplate = function (rpcData) {
        ...
        this.oldvalidJobs = {};
        for (var i in this.validJobs)
            this.oldvalidJobs[i] = this.validJobs[i]
    ...
    this.processShare = function (jobId, previousDifficulty, difficulty, extraNonce1, extraNonce2, nTime, nonce, ipAddress, port, workerName, soln) {
        ...
        if (typeof job === 'undefined' || job.jobId != jobId) {
            job = this.oldvalidJobs[jobId];
            if (typeof job === 'undefined' || job.jobId != jobId) {
                return shareError([21, 'job not found: jobId=', jobId]);
            }
            //_this.emit('log', 'warning', 'in old job list: ' + workerName);
            inOld = true;
        }
        ...
        var buf = new Buffer(soln.slice(6), 'hex');
        if (hashDigest(headerBuffer, buf) !== true) {
            console.log('header: ' + headerBuffer.toString('hex') );
            console.log('sol: ' + buf.toString('hex') );
                return shareError([20, 'invalid solution']);
        }
        //check if block candidate
        if (headerBigNum.le(job.target) && inOld !== true ) {
