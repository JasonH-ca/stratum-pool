...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function StartStratumServer(finishedCallback) {
        ...
        _this.stratumServer.on('tlsclientError', function(error) {
            emitWarningLog(error);
        });
        ...
        _this.stratumServer.on('started', function () {
            ...
            client.on('difficultyChanged', function (diff) {
                ...
                if ( !result.result ) {
                    emitWarningLog('Invalid share from ' + client.getLabel() + ': ' + result.error);
                }
                ...
                if ( typeof(client.fullName) !== 'undefined' ) {
                    emitWarningLog('Rejected incoming connection from ' + client.fullName + ' at ' + client.remoteAddress + ' banned for ' + remainingBanTime + ' more seconds');
                }
            ...
            }).on('setFullname',function(subId, name) {
                _this.stratumServer.setClientFullname(subId, name);
